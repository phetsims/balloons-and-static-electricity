...
const balloonShowAllChargesPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonShowAllChargesPatternStringProperty.value;
const balloonAtPositionPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonAtPositionPatternStringProperty.value;
const singleStatementPatternString = BalloonsAndStaticElectricityFluent.a11y.singleStatementPatternStringProperty.value;
const balloonPicksUpChargesPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonPicksUpChargesPatternStringProperty.value;
const balloonPicksUpMoreChargesPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonPicksUpMoreChargesPatternStringProperty.value;
const balloonPicksUpChargesDiffPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonPicksUpChargesDiffPatternStringProperty.value;
const balloonPicksUpMoreChargesDiffPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonPicksUpMoreChargesDiffPatternStringProperty.value;
const balloonSweaterRelativeChargesPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonSweaterRelativeChargesPatternStringProperty.value;
const lastChargePickedUpPatternString = BalloonsAndStaticElectricityFluent.a11y.lastChargePickedUpPatternStringProperty.value;
const noChargePickupPatternString = BalloonsAndStaticElectricityFluent.a11y.noChargePickupPatternStringProperty.value;
const noChangeInChargesString = BalloonsAndStaticElectricityFluent.a11y.noChangeInChargesStringProperty.value;
const noChangeInNetChargeString = BalloonsAndStaticElectricityFluent.a11y.noChangeInNetChargeStringProperty.value;
const noChargePickupHintPatternString = BalloonsAndStaticElectricityFluent.a11y.noChargePickupHintPatternStringProperty.value;
const noChargePickupWithObjectChargeAndHint = BalloonsAndStaticElectricityFluent.a11y.noChargePickupWithObjectChargeAndHintStringProperty.value;
const releaseHintString = BalloonsAndStaticElectricityFluent.a11y.releaseHintStringProperty.value;
const balloonAddedPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonAddedPatternStringProperty.value;
const balloonRemovedPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonRemovedPatternStringProperty.value;
const balloonAddedWithPositionPatternString = BalloonsAndStaticElectricityFluent.a11y.balloonAddedWithPositionPatternStringProperty.value;
const wallRubbingWithPairsPattern = BalloonsAndStaticElectricityFluent.a11y.wallRubbingWithPairsPatternStringProperty.value;
const wallRubPatternString = BalloonsAndStaticElectricityFluent.a11y.wallRubPatternStringProperty.value;
const wallRubAllPatternString = BalloonsAndStaticElectricityFluent.a11y.wallRubAllPatternStringProperty.value;
const wallRubDiffPatternString = BalloonsAndStaticElectricityFluent.a11y.wallRubDiffPatternStringProperty.value;
...
export default class BalloonDescriber extends Alerter {
  ...
  // a flag that manages how often we should announce a charge
  ...
  private alertNextPickup: boolean;
  ...
  // utterances to be sent to the queue, with a bit of a delay they won't spam
  // the user if they hit the queue to frequently
  private readonly directionUtterance: Utterance;
  private readonly movementUtterance: Utterance;
  private readonly inducedChargeChangeUtterance: Utterance;
  private readonly noChargePickupUtterance: Utterance;
  private readonly chargePickupUtterance: Utterance;

  // utterances for specific events that let us make things assertive/polite
  private readonly grabReleaseUtterance: Utterance;

  // used to determine change in position during a single drag movement, copied to avoid reference issues
  private oldDragPosition: Vector2;

  // monitors position delta in a single drag
  private dragDelta: Vector2;

  // used to watch how much charge is picked up in a single drag action
  private chargeOnStartDrag: number;

  // used to determine how much charge is picked up in a single drag action
  private chargeOnEndDrag: number;

  // time since an alert related to charge pickup has been announced
  private timeSinceChargeAlert: number;

  // every time we drag, mark this as true so we know to describe a lack of charge pick up
  ...
  // flag that indicates that user actions have lead to it being time for a "wall rub" to be
  // described
  private describeWallRub: boolean;
  ...
  private timeSinceReleaseAlert: number;

  // flag that will prevent the firing of the "no movement" alert, set to true with toggling
  // balloon visibility as a special case so we don't trigger this alert when added to the play area
  private preventNoMovementAlert: boolean;

  // distance the balloon has moved since we last sent an alert to the utteranceQueue. After a
  ...
  private distanceSinceDirectionAlert: number;

  // the position of the balloon when we send an alert to the utteranceQueue. After a successful
  // alert, we don't alert again until there is sufficient movement to avoid a pile-up of alerts
  private positionOnAlert: Vector2;

  public constructor( model: BASEModel, wall: WallModel, balloon: BalloonModel, accessibleLabel: string, otherAccessibleLabel: string, nodeToAlertWith: Node ) {
   ...
   */
  private shortMovementFromPointer(): boolean {
   ...
   */
  public getWallRubbingDescription(): string {
   ...
   */
  public getWallRubbingDescriptionWithChargePairs(): string {
   ...
   */
  public getVisibilityChangedDescription(): string {
    let description: string;
